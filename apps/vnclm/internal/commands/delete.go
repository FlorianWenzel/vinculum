...
func deleteMCPCmd() *cobra.Command {
	return &cobra.Command{
		Use:               "mcp <name>",
		Aliases:           []string{"mcpserver"},
		Short:             "Delete MCPServer",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			referencedBy, _ := agentsReferencingMCP(ctx, kc, name)
			if len(referencedBy) > 0 && !state.yes {
				fmt.Printf("mcp %q is referenced by agents: %v\n", name, referencedBy)
			}
			if !confirm("mcp", name) {
				return errf("aborted")
			}
			return withOperator(ctx, kc, func(h *client.HTTP) error {
				if err := h.Delete(ctx, "/api/mcps/"+name); err != nil {
					return err
				}
				fmt.Printf("mcp %q deleted\n", name)
				return nil
			})
		},
	}
...
func agentsReferencingMCP(ctx context.Context, kc *kube.Client, name string) ([]string, error) {
	var agents []client.Agent
	err := withOperator(ctx, kc, func(h *client.HTTP) error {
		return h.GetJSON(ctx, "/api/agents", &agents)
	})
	if err != nil {
		return nil, err
	}
	...
	for _, a := range agents {
		for _, r := range a.Spec.MCPServerRefs {
			if r == name {
				refs = append(refs, a.Metadata.Name)
				break
			}
		}
	}
