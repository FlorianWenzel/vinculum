...
func createProviderCmd() *cobra.Command {
	...
	c.Flags().StringVar(&ptype, "type", "", "provider type (azure|anthropic|openai|opencode|custom)")
	c.Flags().StringToStringVar(&setFlag, "set", nil, "key=value pairs to store in Secret")
	_ = c.RegisterFlagCompletionFunc("type", staticValues("azure", "anthropic", "openai", "opencode", "custom"))
	return c
}
