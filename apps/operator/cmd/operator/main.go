...
func main() {
	...
	settingsStore := appsettings.NewStore(mgr.GetClient(), watchNamespace)
	go startAPIServer(mgr.GetClient(), watchNamespace, cfg, settingsStore)
...
func startAPIServer(k8s client.Client, namespace string, cfg appconfig.Config, settingsStore *appsettings.Store) {
	mux := newAPIMux(k8s, namespace, cfg, settingsStore)
