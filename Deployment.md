...
- Tags: `latest` on default branch, branch name, git tag, `sha-*`, and semver. The `:<appVersion>` (e.g. `:0.6.0`) tag is produced **only by `v*` tag pushes** — main pushes don't claim it, to prevent two builds racing the same tag.
...
Operational meaning:
- Installing chart version `0.6.0` uses app images tagged `0.6.0` by default.
...
hardcoded `:0.6.0`) won't auto-track when the chart's appVersion bumps:
...
2. **Pinned-image** — `kubectl patch agent <name> --type=merge \
   -p '{"spec":{"image":"ghcr.io/florianwenzel/vinculum-agent:0.6.0"}}'`
